...
const Index = () => {
    ...
    useEffect(() => {
        if (token) {
            axios.get('https://bookie.laravel.cloud/api/books', {
            ...
            axios.get('https://bookie.laravel.cloud/api/wishlist', {
                ...
                .then((response) => {
                    if (response.data && Array.isArray(response.data.data)) {
                        setWishlist(response.data.data.map(item => item.book_id));
                    } else {
                        console.error('Unexpected response format:', response.data);
                    }
                })
    ...
    const toggleWishlist = (bookId) => {
        axios.post('https://bookie.laravel.cloud/api/wishlist/toggle', { book_id: bookId }, {
            headers: { Authorization: `Bearer ${token}` }
        })
            .then((response) => {
            ...
            <div className="flex-1 p-6 bg-gray-100">
                <div className="pt-6">
                    ...
                    <div className="flex space-x-4 mb-4">
                        ...
                        <button className="bg-red-500 text-white px-4 py-2 rounded-lg">Trending</button>
                        <button className="bg-red-500 text-white px-4 py-2 rounded-lg" onClick={toggleSortOrder}>
                    ...
                    <div className="grid gap-8 mb-6 lg:mb-16 md:grid-cols-2">
                        {filteredBooks.map((book) => (
                            <div key={book.id} className="relative items-center bg-gray-50 rounded-lg shadow sm:flex dark:bg-gray-800 dark:border-gray-700">
                                ...
                                >
                                    <FaHeart className={wishlist.includes(book.id) ? "text-red-500" : "text-black"} />
                                </button>
                            </div>
                        ))}
                    </div>
